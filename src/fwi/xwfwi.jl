...
Update pa.w
"""
function wfwi!(pa::PFWI; store_trace::Bool=true, extended_trace::Bool=false, time_limit=Float64=2.0*60., 
...
function xwfwi!(pa; max_roundtrips=100, max_reroundtrips=10, PFWIAM_func=nothing, roundtrip_tol=1e-6,
		     optim_tols=[1e-6, 1e-6])

	if(PFWIAM_func===nothing)
		PFWIAM_func=x->PFWIAM(x, optim_tols=optim_tols,name="PFWI with Source Wavelet Estimation",
	...
	f1=x->PFWI.wfwi!(pa, extended_trace=false)
	f2=x->PFWI.xfwi!(pa, extended_trace=false)
	paam=PFWIAM_func([f1, f2])
