...
function update!(
    ...
    ipointloc = [
        Interpolation.indminn(mod.mgrid[i], Float64(point_loc[i]), 1)[1] for
        i = 1:length(mod.mgrid)
    ]
        ...
        temp += [
            (
                (
                    (
                        (
                            (mod.mgrid[2][ix] - ellip_loc[2]) * cos(α) +
                            (mod.mgrid[1][iz] - ellip_loc[1]) * sin(α)
                        )^2 * inv(rads[1]^2) +
                        (
                            (-mod.mgrid[1][iz] + ellip_loc[1]) * cos(α) +
                            (mod.mgrid[2][ix] - ellip_loc[2]) * sin(α)
                        )^2 * inv(rads[2]^2)
                    ) <= 1.0
                ) ? Float64(ellip_pert) : 0.0
            ) for iz = 1:length(mod.mgrid[1]), ix = 1:length(mod.mgrid[2])
        ]
    end
    if (!(rect_pert == 0.0))
        temp += [
            ...
            ) ? rect_pert : 0.0 for iz = 1:length(mod.mgrid[1]),
            ix = 1:length(mod.mgrid[2])
        ]
    end
