...
function updateA!(pa, mpars)
	...
	for j = 2 : pa.nx - 1 # interior x (column) loop (for amount of internal nodes)
	    k = 2 + pa.nz*(j - 1); # initial interior z for this x. To jump from j (new column), need pa.nz*... to proceed to next column vector k
	    for i = 2 : pa.nz - 1  # interior z (row) loop:
		pa.A[k,k - pa.nz] = .5*(mpars[i,j] + mpars[i,j - 1])*ddx; # moving left (and placing actual staggered 2D grid medium value there)
		pa.A[k,k - 1] = .5*(mpars[i,j] + mpars[i - 1,j])*ddz; # moving up the grid
		pa.A[k,k] = -(mpars[i,j] + .5*(mpars[i,j - 1] + mpars[i,j + 1]))*ddx -
		(mpars[i,j] + .5*(mpars[i - 1,j] + mpars[i + 1,j]))*ddz; # diagonal elements: ∇⋅\medpars(x,z)∇ calculated using 5 mpars points
		pa.A[k,k + 1] = .5*(mpars[i,j] + mpars[i + 1,j])*ddz; # moving down the grid
		pa.A[k,k + pa.nz] = .5*(mpars[i,j]+ mpars[i,j + 1])*ddx; # moving right on grid (need pa.nz values to go to next column)
		k = k + 1; # to next z
...
"""
function solve!(field,pa,solflag::Int64) # fields: either field or source
	...
	elseif(solflag==1) # forward problem: source+medium --> fields
		...
		psi_vec=pa.A\src[:]; # direct inverse solve using backslash operator
