...
@parallel function compute_v!(vx, vz, rhoI, dpdx, dpdz, dt)#
    @inn(vx) = @inn(vx) + dt * @av_yi(rhoI) * @all(dpdx)
    @inn(vz) = @inn(vz) + dt * @av_xi(rhoI) * @all(dpdz)
    return
end

@parallel function compute_v!(vx, vy, vz, rhoI, dpdx, dpdy, dpdz, dt)#
    @inn(vx) = @inn(vx) + dt * @av_zi(rhoI) * @all(dpdx)
...
# @parallel_indices (iz, ix) function dpdx!(dpdx, p, fc1, nz, nx)
#     # for ix=2:nx-2
#     # @simd for iz=1:nz
#     dpdx[iz, ix] =
#         (27.e0 * p[iz, ix+1] - 27.e0 * p[iz, ix] - p[iz, ix+2] + p[iz, ix-1]) * fc1
...
# @parallel_indices (iz, ix) function dpdz!(dpdz, p, fc1, nz, nx)
#     # for ix=1:nx
#     # @simd for iz=2:nz-2
#     dpdz[iz, ix] =
#         (27.e0 * p[iz+1, ix] - 27.e0 * p[iz, ix] - p[iz+2, ix] + p[iz-1, ix]) * fc1
#     # end
#     # end
#     return
# end
# @parallel_indices (iz, ix) function dvxdx!(dvxdx, vx, fc1, nz, nx)
#     # for ix=3:nx-1 # ix=1,2,nx are useless, rigid boundary conditions apply on ix=3,nx-1
#     # @simd for iz=1:nz
#     dvxdx[iz, ix] =
#         (27.e0 * vx[iz, ix] - 27.e0 * vx[iz, ix-1] - vx[iz, ix+1] + vx[iz, ix-2]) * fc1
#     # end
#     # end
#     return
# end
# @parallel_indices (iz, ix) function dvzdz!(dvzdz, vz, fc1, nz, nx)
#     # for ix=1:nx
#     # @simd for iz=3:nz-1 # iz=1,2 are usless, boundary conditions on iz=3,nz-1
#     dvzdz[iz, ix] =
#         (27.e0 * vz[iz, ix] - 27.e0 * vz[iz-1, ix] - vz[iz+1, ix] + vz[iz-2, ix]) * fc1
#     # end
#     # end
#     return
# end
