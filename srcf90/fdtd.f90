...
! >>>>>>> snapsshots and illumination and gradients
        snaps_in, & ! input snaps shots in the order [mesh_nz,mesh_nx,tim_nt,src_nseq] 
        snaps_out, & ! output snaps shots in the order [mesh_nz,mesh_nx,tim_nt,src_nseq]
...
!         updated on 25 July 2014
        ...
        implicit none
character(len=1, kind=C_char), intent(in)               :: jobname(*), mesh_abs_trbl(*), src_flags(*), recv_flags(*)
real, intent(in)                                        :: tim_del, snap_save_at
integer, intent(in)                                     :: npropwav
integer, intent(in)                                     :: tim_nt, src_nt, src_nsmul, src_nseq, recv_n
real, dimension(:,:), intent(in)                        :: modtt, modrr
...
real, dimension(:), intent(out)                         :: recv_out
...
optional                                                :: recv_out, recv_flags, recv_n, recv_x, recv_z
...
!
!! dimension checks
!call check_dimension("fd2_mod:",modtt,mesh_nz,mesh_nx)
!call check_dimension("fd2_mod:",modrr,mesh_nz,mesh_nx)
!if(tim_nt .lt. src_nt) call abort_msg("fd2_mod: tim_nt .lt. src_nt")
!call check_dimension("fd2_mod: size src_wavelets", src_wavelets, src_nt*src_nsmul*src_nseq)
...
!endif
!if(present(recv_out)) then
!        if(.not.(present(recv_n))) call abort_msg("fd2_mod: need recv_n")
!        if(.not.(present(recv_flags))) call abort_msg("fd2_mod: need recv_flags")
!        call check_dimension("fd2_mod: size recv_out", recv_out, tim_nt*recv_n*src_nseq)
!        recv_out = rzero_de
!endif
!if(present(snaps_out)) then
!        call check_dimension("fd2_mod: size snaps_out", snaps_out,  mesh_nz,mesh_nx,tim_nt, src_nseq)
!        snaps_out = rzero_de
!endif
!if(present(snaps_in)) then
!        call check_dimension("fd2_mod: size snaps_in", snaps_in, mesh_nz, mesh_nx,tim_nt, src_nseq)
!endif
!
...
!                do it = 1, src_nt-1
!                        if(index(src_flags,'[TIME_REVERSE]') .ne. 0) then
!                                source_term = &
...
!                        endif
!                        if(index(src_flags,'[IRATE]') .eq. 0) then
...
!nx    = mesh_nx+2*mesh_na_pml
!nz    = mesh_nz+2*mesh_na_pml
!
!! assign variables
!deltax=mesh_dx
...
!lambda = extend((var%vel * var%vel * var%rho), mesh_na_pml, mesh_nx, mesh_nz)
!rho = extend((var%rho), mesh_na_pml, mesh_nx, mesh_nz)
!
!! rhoI on p frid
...
!!$OMP SHARED(src_nseq,src_wav_mat,src_flags, var, grad, mesh_nz, mesh_nx, nz, nx, snaps_in, &
!!$OMP mesh_x, mesh_z, &
!!$OMP snaps_out, tim_nt, src_nt, src_nsmul, mesh_na_pml, deltax, deltaz, deltax24I, deltaz24I, tim_del, tim_delI, & 
!!$OMP src_x, src_z, recv_n, recv_z, recv_x, snap_save_at, recv_out, recv_flags, &
...
!        do issmul = 1, src_nsmul
!                if(index(src_flags,'[BILINEAR]') .ne. 0) then
!                        maskX = .true.
!                        issmulxtemp1 = &
...
!                        (src_z(issmul + (isseq-1)*src_nsmul)-mesh_z(issmulz1(issmul)))*denomsrcI(issmul)
!
!                elseif(index(src_flags,'[NEAREST_NEIGH]') .ne. 0) then
!                        issmulx1(issmul) = &
...
!                do irec = 1, recv_n
!                        if(index(recv_flags, '[BILINEAR]') .ne. 0) then
!                                maskX = .true.
!                                irecxtemp1 = &
!                                minloc(array = &
...
!                                        ((recv_x(irec + (isseq-1) * recv_n)-mesh_x(irecx1(irec)))*&
!                                        (recv_z(irec + (isseq-1) * recv_n)-mesh_z(irecz1(irec))))*denomrecI(irec)
!                        elseif(index(recv_flags,'[NEAREST_NEIGH]') .ne. 0) then
!                                irecx1(irec) = &
!                                minloc(array = &
...
!                                        * deltax24I * deltaz24I
!                        p(mesh_na_pml+issmulz1(issmul), mesh_na_pml+issmulx1(issmul)) = p(mesh_na_pml+issmulz1(issmul), mesh_na_pml+issmulx1(issmul)) &
!                                + svalue * src_spray_weights(issmul,1) * lambda(mesh_na_pml+issmulz1(issmul), mesh_na_pml+issmulx1(issmul))  
!                        p(mesh_na_pml+issmulz1(issmul), mesh_na_pml+issmulx2(issmul)) = p(mesh_na_pml+issmulz1(issmul), mesh_na_pml+issmulx2(issmul)) &
!                                + svalue * src_spray_weights(issmul,2) * lambda(mesh_na_pml+issmulz1(issmul), mesh_na_pml+issmulx2(issmul))
!                        p(mesh_na_pml+issmulz2(issmul), mesh_na_pml+issmulx1(issmul)) = p(mesh_na_pml+issmulz2(issmul), mesh_na_pml+issmulx1(issmul)) &
!                                + svalue * src_spray_weights(issmul,3) * lambda(mesh_na_pml+issmulz2(issmul), mesh_na_pml+issmulx1(issmul))
!                        p(mesh_na_pml+issmulz2(issmul), mesh_na_pml+issmulx2(issmul)) = p(mesh_na_pml+issmulz2(issmul), mesh_na_pml+issmulx2(issmul)) &
!                                + svalue * src_spray_weights(issmul,4) * lambda(mesh_na_pml+issmulz2(issmul), mesh_na_pml+issmulx2(issmul))
!		enddo
!
...
!                if(present(grad) .and. (it .ne. tim_nt) .and. (it .ne. 1)) then
!
!                        ! p at [it], pp at [it-1]
!                        ! dpdx and dpdz at [it]
...
!                                        (snaps_in(iz,ix,tim_nt-it,isseq)-snaps_in(iz,ix,tim_nt-it+1,isseq)) * tim_delI * &
!                                        (p(iz+mesh_na_pml,ix+mesh_na_pml)-pp(iz+mesh_na_pml,ix+mesh_na_pml)) * tim_delI
!                        endforall
!                        
!
...
!                                        dpFdx(iz,ix) = &
!                                                ((27.e0*snaps_in(iz,ix+1,tim_nt-it,isseq)-27.e0*snaps_in(iz,ix,tim_nt-it,isseq)-&
!                                                snaps_in(iz,ix+2,tim_nt-it,isseq)+snaps_in(iz,ix-1,tim_nt-it,isseq)) &
!                                                * (deltax24I))     
!                                        ! calculating dpFdz are on vz grid
!                                        dpFdz(iz,ix) = &
!                                                ((27.e0*snaps_in(iz+1,ix,tim_nt-it,isseq)-27.e0*snaps_in(iz,ix,tim_nt-it,isseq)-&
!                                                snaps_in(iz+2,ix,tim_nt-it,isseq)+snaps_in(iz-1,ix,tim_nt-it,isseq)) &
!                                                * (deltaz24I))     
!                                endforall
...
!!                                        rhalf * (dpdx(iz+mesh_na_pml,ix+mesh_na_pml) + dpdx(iz+mesh_na_pml,ix-1+mesh_na_pml))) + &
!!                                                        (rhalf * (dpFdz(iz,ix) + dpFdz(iz-1,ix)) *&
!!                                        rhalf * (dpdz(iz+mesh_na_pml,ix+mesh_na_pml) + dpdz(iz-1+mesh_na_pml,ix+mesh_na_pml))))
!                                        grad_rhovxI(iz,ix) = grad_rhovxI(iz,ix) - dpdx(iz+mesh_na_pml,ix+mesh_na_pml)*dpFdx(iz,ix) 
!                                        grad_rhovzI(iz,ix) = grad_rhovzI(iz,ix) - dpdz(iz+mesh_na_pml,ix+mesh_na_pml)*dpFdz(iz,ix) 
!                                endforall
!                        endif
!                                
!                endif
...
!                !                saved_snap(:,:) = p(mesh_na_pml+1:mesh_nz+mesh_na_pml,1+mesh_na_pml:mesh_nx+mesh_na_pml) ;
!                !        endif
!                !endif
!
...
!                if(present(snaps_out)) then
!                        forall(ix=1:mesh_nx,iz=1:mesh_nz) 
!                                snaps_out(iz,ix,it,isseq) = p(mesh_na_pml+iz,mesh_na_pml+ix)
!                        endforall
!                end if
!
